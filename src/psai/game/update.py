...
def update_state(
    ...
    #--------------#
        ...
        if state.active_player == 0: #TODO change this, the "first player" rotates each hour. Another variable?
            ...
            # Increment the day counter if the sun position is at 0
            if state.sun_pos == 0:
                state.day += 1
...
def distribute_light(state: State) -> None:
    ...
    for cell in state.board_state:
        if cell.size is not None and cell.owner is not None:
            # Determine the light received based on tree size
            if cell.size == 'seed':
                light_received = 0
            elif cell.size == 'small':
                light_received = 1
            elif cell.size == 'medium':
                light_received = 2
            elif cell.size == 'large':
                light_received = 3
            else:
                raise ValueError(f"Unknown tree size: {cell.size}")

            # Check if the tree is shaded
            # We check the cells in the direction of the sun as the shaded locations 
            # if the sun was in the opposite direction (state.sun_pos+3) % 6.
            shaded_locs = get_shaded_locs(cell.loc[0], cell.loc[1], (state.sun_pos+3) % 6)
            is_shaded = False
            shading_sizes = {
            ...
            for iloc, loc in enumerate(shaded_locs, start=1):
                if loc is not None:
                    shader_cell = state.get_cell_at_loc(loc)
                    if shader_cell.size is None:
                        continue
                    if shader_cell.size in shading_sizes[iloc]:
                        is_shaded = True
                        break

            if not is_shaded:
                state.player_stats[cell.owner].light += light_received